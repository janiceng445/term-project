...
void boss::attackMove() {
	...
	if (this->isAlive) {
		...
		if (this->y >= 600) { //move boss downwards if boss gets too high up
			this->aniSprite.move((((0.3 - 0.1) * (500 - this->HP)) / 500) + 0.1, -0.5);  
			return;
		}
		if (this->y <= 150) { //move boss upwards if boss gets too low
			this->aniSprite.move((((0.3 - 0.1) * (500 - this->HP)) / 500) + 0.1, 0.5);
			return;
		}
		...
		this->aniSprite.move((((0.3 - 0.1) * (500 - this->HP)) / 500) + 0.1, 3 / ((65 + this->y) - sf::Mouse::getPosition(*window).y));
