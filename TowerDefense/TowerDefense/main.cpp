...
int main()
{
	...
	//////////////////////////////// Wave of enemies /////////////////////////////////
	int waveRound = 0;
	randSkellyTimer = SKELLY_SPWN_TIMER + (rand() % 6) - 3;
	randRhinoTimer = RHINO_SPWN_TIMER + (rand() % 6) - 3;
	randLancerTimer = LANCER_SPWN_TIMER + (rand() % 6) - 3;
	randDemonTimer = DEMON_SPWN_TIMER + (rand() % 6) - 3;
	...
	while (window.isOpen())
	{
		...
		if (!paused)
		{
			...
			// Parameters: maximum spawns, time threshold, spawn timer, wave vector, window, animation vector, dmg, hp, boundary, target's hp, name, score
			runSpawners(&skellyMax, randSkellyTimer, SKELLY_SPWN_TIMER, &wave, &window, &skellyAni, skelly_DMG, skelly_HP, boundary, &targetHP, name.at(0), &gameScore);
			runSpawners(&rhinoMax, randRhinoTimer, RHINO_SPWN_TIMER, &wave, &window, &rhinoAni, rhino_DMG, rhino_HP, boundary, &targetHP, name.at(1), &gameScore);
			runSpawners(&lancerMax, randLancerTimer, LANCER_SPWN_TIMER, &wave, &window, &lancerAni, lancer_DMG, lancer_HP, boundary, &targetHP, name.at(2), &gameScore);
			runSpawners(&demonMax, randDemonTimer, DEMON_SPWN_TIMER, &wave, &window, &demonAni, demon_DMG, demon_HP, boundary, &targetHP, name.at(3), &gameScore);
...
void runSpawners(int* maxSpawn, int &timeThreshold, int spwn_timer, std::vector<Monster*>* wave, sf::RenderWindow* win, std::vector<Animation>* ani, int dmg, int hp, int boundary, int* targetHP, std::string name, Score* score) {
	if (gameTimer + (int)clockGame.getElapsedTime().asSeconds() >= timeThreshold && *maxSpawn != 0) {
		if (name == "Lancer") {
			Lancer* spawn = new Lancer(win, *ani, dmg, hp, score);
			spawn->setTarget(boundary, *&targetHP);
			wave->push_back(spawn);
		...
		*maxSpawn -= 1;
		timeThreshold += spwn_timer + (rand() % 6) - 3;
	}
}